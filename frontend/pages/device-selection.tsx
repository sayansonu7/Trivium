...
export default function DeviceSelectionPage() {
  ...
  useEffect(() => {
    ...
    const deviceLimitCookie = cookies.find(c => c.trim().startsWith('device_limit_data='));
    
    if (deviceLimitCookie) {
      try {
        const data = JSON.parse(decodeURIComponent(deviceLimitCookie.split('=')[1]));
  ...
  const handleForceLogin = async (sessionId: string) => {
    ...
    try {
      ...
      // Clear cookies
      document.cookie = 'pending_login=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
      document.cookie = 'device_limit_data=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
